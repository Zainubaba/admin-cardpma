...
<div style="margin-top: 20px; text-align: center;">
  <button id="forwardButton" class="btn btn-success">Forward Selected</button>
</div>
...
<script>
  $('#forwardButton').click(function() {
    ...
    if (!confirm('Are you sure you want to forward the selected rows?')) {
      return;
    }

    var selectedIds = selectedRows.map(function(row) {
      return row.uid; // 🔑 Change 'uid' to match your unique ID field
    });

    console.log('Selected IDs:', selectedIds);

    $.ajax({
      url: '/forward2', // 🔗 Replace with your Laravel API route
      type: 'post',
      data: {
        ids: selectedIds
      },
