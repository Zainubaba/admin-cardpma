...
@endif
                        ...
                        <form method="POST" action="{{ route('Institute.storeuserinstitute') }}">
                            @csrf
...
$(document).ready(function() {
    $('#org_name').on('change', function() {
        if ($(this).val() === 'other') {
            ...
            $('#other_org_name').val(''); // clear input
        }
    });
});
</script> --}}
            ...
            $(() => {
                $('#gridContainer').dxDataGrid({
...

                    {
                            ...
                            cssClass: 'bullet',
                            width:50,
                            allowSorting: false, // optional: disables sorting on this column
...

                    {
...

                     {
...

                        {
                            ...
                            cssClass: 'bullet',
                            cellTemplate: function(container, options) {
                                const button = $('<a style="color:white;" class="btn btn-sm btn-success">')
                                    .text('Add Institute')
                                    .on('click', function () {
                                        const instituteData = {
                                            district_id: options.data.district,
                                            tehsil_id: options.data.tehsil,
                                            institute_type: options.data.institute_type,
                                            edu_level: options.data.edu_level,
                                            hod_id:options.data.hod_id,
                                            org_name: options.data.org_name, // adjust according to your actual field
                                            emis: options.data.emis,         // optional fields
                                            _token: $('meta[name="csrf-token"]').attr('content') // CSRF token
                                        };

                                        $.ajax({
                                            url: '/addorganization', // route to your controller
                                            method: 'POST',
                                            data: instituteData,
                                            success: function(response) {
                                                alert('Institute added successfully!');
                                                // Optionally refresh the grid or disable the button
